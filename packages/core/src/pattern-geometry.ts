...
// the panel at any aspect ratio (every panel format is taller than wide, so
// size = panelHeightMm(doc.format) — 128.5 for 3U, 39.65 for 1U). SINGLE
// source of truth for this formula — used by createDefaultDoc, the
// serialization migration (serialize.ts), the pattern picker's add-new-layer
// path, and the inspector's "Cover panel" reset (interaction follow-up sub).
// Do NOT duplicate it anywhere.
