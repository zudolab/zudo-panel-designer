...
import type { PanelFormat, PcbMaterial } from '@zpd/core';
...
/**
 * Matches `download.ts`'s `zpd-panel-<hp>hp.json` stem, extended with format
 * and material now that both vary (#231). #229's shared download-filename
 * helper takes over the construction when it lands — whichever side merges
 * second reconciles here.
 */
export function gerberZipFilename(format: PanelFormat, material: PcbMaterial, hp: number): string {
  return `zpd-panel-${format.toLowerCase()}-${hp}hp-${material}-gerber.zip`;
}
