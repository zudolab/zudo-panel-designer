 ...
 *
 * Measured ${date}: ${names.length}/${PATTERN_GENERATORS.length} generators, across default/min/max parameters.
 */
...
(GENERATE ? describe : describe.skip)('generate pattern-union-unreliable.generated.ts', () => {
  it(
    ...
    async () => {
      ...
      const names = await measureUnionUnreliableNames(engine, PATTERN_GENERATORS);
