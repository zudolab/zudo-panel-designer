...
すべてのレイヤーの互換用 `color`（および `path` レイヤーの `fill`／`stroke`
有効状態）は `ColorIndex`（`0`、`1`、`2` のいずれか）を使います。v5
では所属する固定コンテナが正式な値であり、null でない各 paint フィールドは
そのマテリアルのインデックスへ正規化されます。対応は
`packages/core/src/palette.ts` で一度だけ定義されます。
