...
All four exclude **pattern layers** from the operated-on set — patterns are the single panel-wide background (see [Tools → Layer eligibility matrix](./tools.mdx#layer-eligibility-matrix)), so they are never copyable, cuttable, duplicable, or included by Select All. Copy, cut, and duplicate act on the **current selection**, so they're no-ops when nothing eligible is currently selected. Select All doesn't read the current selection at all — it always selects every non-pattern layer in the document, so it's a no-op only when the document has no non-pattern layers.
