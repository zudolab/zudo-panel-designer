...
- **`chord` absent** ("chordless") — the command has no keyboard shortcut at all; it's reachable only from the command palette. Align, Distribute, New Panel, Import/Download JSON, Download Gerber, the three zoom actions, and Browse Google Fonts are all chordless by design.
...
Chordless commands (Align, New Panel, Import/Download JSON, Download Gerber, zoom, Browse Google Fonts) do **not** appear here — they have nothing to show. Reach for the command palette to run those.
