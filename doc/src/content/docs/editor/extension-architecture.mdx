...
`ToolContext.selectedIds` is the **single source of truth** for what's selected — a `readonly string[]` that is always normalized (`selection.ts`): de-duplicated, filtered to ids that still exist in the current document (a stale id after a delete/undo drops out), and in **document order** — the layers-array order, not the order the ids were clicked. So chrome and inspectors always iterate a stable, valid list.
