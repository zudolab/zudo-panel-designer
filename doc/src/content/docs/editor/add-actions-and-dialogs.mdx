...
The [sidebar Help panel](./interface.mdx#right-sidebar) **complements** this dialog rather than replacing it: the Help panel explains the one tool that is currently active (its per-tool `description`), while the Shortcuts dialog remains the single, always-available reference for the full key/mouse map across every tool. Reach for the dialog when you want the whole cheat sheet; glance at the Help footer when you want to know what the tool in your hand does.
