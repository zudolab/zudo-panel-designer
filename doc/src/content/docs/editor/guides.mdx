...
Guides round-trip through the exported `PanelConfig`. Serialization was bumped to **v2** to carry the new top-level `guides` array; a v1 (or any guide-less) config still loads cleanly, with `guides` defaulting to `[]`. The field-by-field rules — a guide needs a valid `orientation` and a finite `position` or it is dropped, a missing `id` is stamped, `hidden` is preserved — are covered in [PanelConfig Format → Versioning](../export/panel-config-format.mdx#versioning) and [Defensive Parsing → Guides](../export/defensive-parsing.mdx#guides).
