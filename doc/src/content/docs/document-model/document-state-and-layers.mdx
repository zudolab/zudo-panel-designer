...
**v2** added the top-level `guides` array. A v1 config (or any config missing `guides`) still loads cleanly, with `guides` defaulting to `[]`: `parsePanelConfig` doesn't branch on `version` — every field defends itself — so old files need no dedicated migration path. See [PanelConfig Format → Versioning](../export/panel-config-format.mdx#versioning).
